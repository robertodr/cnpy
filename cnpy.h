...
struct NpyArray {
  ...
  explicit NpyArray(const std::vector<size_t> & _shape,
                    size_t _word_size,
                    bool _fortran_order)
      : shape(_shape),
        word_size(_word_size),
        fortran_order(_fortran_order),
        num_vals(std::accumulate(shape.begin(),
                                 shape.end(),
                                 1,
                                 std::multiplies<size_t>())),
        data_holder(num_vals * word_size, char(0)) {}

  template <typename T> T * data() {
    return reinterpret_cast<T *>(data_holder.data());
  }

  template <typename T> std::vector<T> as_vec() {
    const T * p = data<T>();
    return std::vector<T>(p, p + num_vals);
  }

  size_t num_bytes() { return data_holder.size(); }
...
void npz_save(std::string zipname,
  ...
  global_header += (uint32_t)global_header_offset; // relative offset of local file
...
template <typename T>
void npy_save(std::string fname, T data, std::string mode = "w") {
  std::vector<size_t> shape;
  shape.push_back(1);
